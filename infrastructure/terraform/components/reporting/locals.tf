locals {
  ...
  powerbi_gateway_script = templatefile("${path.module}/templates/cloudinit_config.tmpl", {
    ...
    recovery_key        = aws_ssm_parameter.powerbi_gateway_recovery_key[0].name
    client_secret       = aws_ssm_parameter.powerbi_gateway_client_secret[0].name
    client_id           = aws_ssm_parameter.powerbi_gateway_client_id[0].name
    tenant_id           = aws_ssm_parameter.powerbi_gateway_tenant_id[0].name

  })

  use_core_glue_catalog_resources = length(var.non_prod_core_account_ids) > 0 ? true : false

  core_glue_catalog_resources = local.use_core_glue_catalog_resources ? flatten([
    for account_id in var.non_prod_core_account_ids : [
